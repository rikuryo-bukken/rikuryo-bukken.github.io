...
ページの変更をmainブランチへマージすることでページが公開されるが、ページ公開の際には、変更の意図をマージのメッセージに記載すること。これは、重なっていく変更の意図を明確にして蓄積していくことで、サイトが悪い方向へと逆戻りするのを防ぐことが目的である。
...
```html
<dt>{game title}</dt>
<dd><a href="/{grade}/{new_game}" aria-label="{game title}をプレイ"><img src="images/{grade}/{new_game}.webp" alt="{game title}のプレイ画面"></a></dd>
<dd>ゲームの説明</dd>
...
id属性がpagerのタグ内には、ulタグ（順序なしリスト）またはolタグ（順序付きリスト）を追加しても削除してもよい。基本的には、最初ののolタグにはパンくずリスト（記述の仕方は138期のページなどを参考のこと）を、その次のulタグにはゲーム集及びサイトマップのリンクを記述する。なお、ulタグとolタグ以外を追加するのは不可である。