...
id属性がpagerのタグ内には、ulタグ（順序なしリスト）またはolタグ（順序付きリスト）を追加しても削除してもよい。基本的には、最初のolタグにはパンくずリスト（記述の仕方は138期のページなどを参考のこと）を、その次のulタグにはゲーム集及びサイトマップのリンクを記述する。なお、ulタグとolタグ以外を追加するのは不可である。